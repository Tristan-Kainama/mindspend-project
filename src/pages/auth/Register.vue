...
const handleRegister = async () => {
  ...
  try {
    ...
    console.error(err)
    errorMsg.value = err.message || 'Register gagal'
  } finally {
    loading.value = false
  }
}
...
<template>
  <div class="bg-gradient-to-b from-[#2678ff] to-[#00b0da] relative w-full h-full flex flex-col">
    ...
    <p class="absolute font-geologica font-normal h-[19px] leading-[normal] left-[97px] not-italic text-[12.664px] text-black top-[590px] w-[238px]" style="font-variation-settings: 'CRSV' 0, 'SHRP' 0">
      Confirm Password
    </p>

    <!-- Confirm Password Input -->
    <div class="absolute bg-white border border-[#d9d9d9] border-solid h-[60px] left-[97px] rounded-[9px] top-[614px] w-[253px]" />
    <input
      v-model="confirmPassword"
      type="password"
      placeholder=""
      class="absolute bg-transparent border-none h-[60px] left-[97px] outline-none px-4 rounded-[9px] text-black top-[614px] w-[253px]"
    />

    <!-- Register Button -->
    <button
      @click="handleRegister"
      class="absolute bg-[#2678ff] h-[60px] left-[97px] rounded-[9px] top-[720px] w-[253px] cursor-pointer hover:bg-[#1d5dd4] transition-colors"
    >
    ...
    <!-- Sign Up Link -->
    <div class="absolute top-[800px] w-full flex justify-center gap-2 text-[14px]">
      <span>Already have an account?</span>
      <span
        @click="$router.push('/login')"
        class="text-[#2678ff] underline cursor-pointer hover:text-[#1d5dd4]"
      >
        Login
      </span>
    </div>
    ...
    <div class="absolute mt-8 space-y-4 top-[850px] left-[35px] w-[361px]">
